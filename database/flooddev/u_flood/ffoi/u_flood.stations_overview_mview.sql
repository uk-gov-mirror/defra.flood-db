 ...
 SELECT s.rloi_id,
   ...
   FROM u_flood.station_split_mview s
     LEFT JOIN ( SELECT p_rank.rloi_id,
            ...
            p_rank.formatted_value_timestamp,
            p_rank.error,
            p_rank.value_rank,
            p_rank.parent_rank
           FROM ( SELECT p.rloi_id,
                    ...
                    v_rank.formatted_value_timestamp,
                    v_rank.error,
                    v_rank.value_rank,
                   ...
                   FROM ( SELECT telemetry_value.telemetry_value_id,
                            telemetry_value.telemetry_value_parent_id,
                            telemetry_value.value,
                            telemetry_value.processed_value,
                            telemetry_value.value_timestamp,
                            telemetry_value.formatted_value_timestamp,
                            telemetry_value.error,
                            rank() OVER (PARTITION BY telemetry_value.telemetry_value_parent_id ORDER BY telemetry_value.value_timestamp DESC) AS value_rank
                           FROM u_flood.telemetry_value) v_rank
                     JOIN u_flood.telemetry_value_parent p ON v_rank.telemetry_value_parent_id = p.telemetry_value_parent_id
     ...
     LEFT JOIN ( SELECT s_1.rloi_id,
            s_1.qualifier
           FROM u_flood.station_split_mview s_1
             JOIN u_flood.telemetry_value_parent p ON s_1.rloi_id = p.rloi_id AND (s_1.qualifier = 'u'::text AND lower(p.qualifier) !~~ '%downstream%'::text OR s_1.qualifier = 'd'::text AND lower(p.qualifier) ~~ '%downstream%'::text)
             JOIN u_flood.telemetry_value v ON p.telemetry_value_parent_id = v.telemetry_value_parent_id
     ...
     LEFT JOIN u_flood.ffoi_station fs on fs.rloi_id = s.rloi_id
WITH DATA;
