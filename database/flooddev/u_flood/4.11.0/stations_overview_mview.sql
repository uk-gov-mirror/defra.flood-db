...
latest_value_summaries_with_previous_value AS (
  ...
  SELECT
    ...
    value_timestamp,
    error,
    avg(ranked_all_value_summaries.processed_value) OVER (
      PARTITION BY ranked_all_value_summaries.rloi_id, ranked_all_value_summaries.qualifier
      ORDER BY ranked_all_value_summaries.rloi_id, ranked_all_value_summaries.qualifier, ranked_all_value_summaries.value_timestamp
      ROWS BETWEEN 4 PRECEDING AND 1 PRECEDING
    ) as previous_value,
    parent_rank
  FROM ranked_all_value_summaries
  WHERE parent_rank in (1,2,3,4,5)
),
...
SELECT s.rloi_id,
    ...
    rb.rloi_id IS NOT NULL AS por_max_breached,
        ...
        EXISTS (
          SELECT 1
          FROM u_flood.ffoi_max f
          WHERE f.telemetry_id = s.telemetry_id
        ) AS forecast,
        CASE
